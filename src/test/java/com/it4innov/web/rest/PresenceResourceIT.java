...
class PresenceResourceIT {
    ...
    @Test
    @Transactional
    void getAllPresencesByAdherantIsEqualToSomething() throws Exception {
        Personne adherant;
        if (TestUtil.findAll(em, Personne.class).isEmpty()) {
            presenceRepository.saveAndFlush(presence);
            adherant = PersonneResourceIT.createEntity(em);
        } else {
            adherant = TestUtil.findAll(em, Personne.class).get(0);
        }
        em.persist(adherant);
        em.flush();
        presence.setAdherant(adherant);
        presenceRepository.saveAndFlush(presence);
        Long adherantId = adherant.getId();

        // Get all the presenceList where adherant equals to adherantId
        defaultPresenceShouldBeFound("adherantId.equals=" + adherantId);

        // Get all the presenceList where adherant equals to (adherantId + 1)
        defaultPresenceShouldNotBeFound("adherantId.equals=" + (adherantId + 1));
    }
