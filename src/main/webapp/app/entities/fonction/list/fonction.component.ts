...
import { EntityArrayResponseType, FonctionService } from '../service/fonction.service';
...
export class FonctionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFonction): number => this.fonctionService.getFonctionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fonctions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFonction[] | null): IFonction[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.fonctionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
