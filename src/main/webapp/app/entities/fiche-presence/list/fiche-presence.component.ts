...
import { EntityArrayResponseType, FichePresenceService } from '../service/fiche-presence.service';
...
export class FichePresenceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFichePresence): number => this.fichePresenceService.getFichePresenceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fichePresences = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFichePresence[] | null): IFichePresence[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.fichePresenceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
