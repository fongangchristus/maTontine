...
import { EntityArrayResponseType, AdhesionService } from '../service/adhesion.service';
...
export class AdhesionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAdhesion): number => this.adhesionService.getAdhesionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.adhesions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAdhesion[] | null): IAdhesion[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.adhesionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
