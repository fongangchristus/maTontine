...
import com.it4innov.service.PresenceQueryService;
...
/**
 * REST controller for managing {@link com.it4innov.domain.Presence}.
 */
@RestController
@RequestMapping("/api")
public class PresenceResource {
    ...
    public ResponseEntity<List<PresenceDTO>> getAllPresences(
        ...
        log.debug("REST request to get Presences by criteria: {}", criteria);
        Page<PresenceDTO> page = presenceQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /presences/count} : count all the presences.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/presences/count")
    public ResponseEntity<Long> countPresences(PresenceCriteria criteria) {
        log.debug("REST request to count Presences by criteria: {}", criteria);
        return ResponseEntity.ok().body(presenceQueryService.countByCriteria(criteria));
    }
